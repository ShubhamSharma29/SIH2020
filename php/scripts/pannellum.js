			...
			if (a || ('multires' != z && 'cubemap' != z)) {
				if ('multires' != z)
					...
					for (
						d = 1;
						d < m.maxLevel &&
						a.drawingBufferWidth > m.tileResolution * Math.pow(2, d - 1) * Math.tan(k / 2) * 0.707;
...
window.pannellum = (function(E, g, p) {
	function Ba(s, k) {
		...
		function wa(a, b) {
			...
			n &&
				b &&
				((ba = 'undefined' !== typeof performance && performance.now() ? performance.now() : Date.now()), F());
		}
		function ga() {
			if (G) {
				var a = !1,
			...
			if (null !== a && '' !== a && k.scenes && k.scenes[a]) {
				...
				for (f in e)
					if (e.hasOwnProperty(f))
						if ('strings' == f)
							for (c in e.strings) e.strings.hasOwnProperty(c) && (b.strings[c] = D(e.strings[c]));
					...
					try {
						s.requestFullscreen
							...
							: s.mozRequestFullScreen
								? s.mozRequestFullScreen()
								: s.msRequestFullscreen ? s.msRequestFullscreen() : s.webkitRequestFullScreen();
					} catch (a) {}
		}
		function d(a) {
		...
		function y(a, c, d, h, g) {
			...
			m =
				'same' === d
					? b.yaw
					: 'sameAzimuth' === d ? b.yaw + (b.northOffset || 0) - (k.scenes[a].northOffset || 0) : d;
			q = 'same' === h ? b.hfov : h;
			e();
			H(a);
		...
		function I(a, b) {
			...
			return 0 === c.indexOf('javascript:') || 0 === c.indexOf('vbscript:')
				...
				: b && 0 === c.indexOf('data:') ? (console.log('Data URI removed from link.'), 'about:blank') : a;
		}
		function ab(a) {
